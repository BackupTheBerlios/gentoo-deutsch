...
# $Header: /home/xubuntu/berlios_backup/github/tmp-cvs/gentoo-deutsch/Repository/ebuilds/media-libs/xine-lib/xine-lib-1_rc3-r1.ebuild,v 1.1 2003/12/24 13:45:22 rootshell Exp $
...
src_unpack() {
	...
	# here we introduce a new USE-VAR (vdr)
        if use vdr
        then
                einfo "patching xine-lib for vdrplugin-xine"
                bzcat ${FILESDIR}/vdr-xine.diff.bz2 | patch -p0
	else
...
src_compile() {
        if use vdr
        then
                einfo "re-running autogen.sh"
                ./autogen.sh noconfig
		# if USE="vdr" we're patching here, as our previous autogen run has overwritten the configure script
		epatch ${FILESDIR}/protect-CFLAGS.patch-${PV} || die
        fi
